...
namespace TaskManagement.Business.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendRegistrationConfirmationEmailAsync(string toEmail, string firstName, string confirmationLink)
        {
            ...
            await SendEmailAsync(toEmail, "Email Confirmation for Azure Task Management", htmlContent, true);
        }
        ...
        private async Task SendEmailAsync(string toEmail, string subject, string body, bool isBodyHtml = false)
        {
            try
            {
                ...
                {
                    UseDefaultCredentials = false, // important: ensure we use the supplied credentials
